...
func TestBlockchain(t *testing.T) {
	...
	t.Run("Should retore blockchain", func(t *testing.T) {
		once = *new(sync.Once)
		dbStorage = fakeDB{
			fakeLoadChain: func() []byte {
		...
		bc := Blockchain()
		if bc.Height != 2 {
			t.Errorf("Blockchain() should retore a blockchain with a height of %d, got %d", 2, bc.Height)
		}
	})
}
...
func TestAddPeerBlock(t *testing.T) {
	...
	if bc.CurrentDifficulty != 2 || bc.Height != 2 || bc.NewestHash != "test" {
		t.Error("AddPeerBlock() shoud mutate the blockchain")
	}
}
...
func TestReplace(t *testing.T) {
	...
	bc.Replace(blocks)
	if bc.CurrentDifficulty != 2 || bc.Height != 2 || bc.NewestHash != "test" {
		t.Error("Replace() shoud mutate the blockchain")
	}
}
