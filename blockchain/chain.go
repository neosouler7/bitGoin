...
func (b *blockchain) AddBlock() *Block {
	block := createBlock(b.NewestHash, b.Height+1, getDifficulty(b)) // create & persist on db
...
func (b *blockchain) AddPeerBlock(newBlock *Block) {
	...
	persistBlockchain(b)
	persisBlock(newBlock)

	for _, tx := range newBlock.Transactions { // clear mempool by txId
		_, ok := m.Txs[tx.Id]
		if ok {
			delete(m.Txs, tx.Id)
		}
